    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.max_steps > 0 and global_step > args.max_steps:
                ...
                break
        if args.populate_finetune:
            epoch_num += 1
            if epoch_num % 3 == 0:
                pruning_model(model, 0)
                mask_dict = {}
                weight_dict = {}
                ...
                # torch.save(mask_dict, args.output_dir + '/mask' + str(epoch_num) + '.pt')
                torch.save(weight_dict, args.output_dir +'/weight' + str(epoch_num) + '.pt')
        if args.max_steps > 0 and global_step > args.max_steps:
...
def main():
    ...
    parser.add_argument("--rewind_epoch", type=int)
    parser.add_argument("--populate_finetune", action="store_true")
    ...
    if args.do_train:
        ...
        if args.populate_finetune:
            print('Populating Finetuned Epoch Checkpoints')
            evaluate(args, model, tokenizer)
            global_step, tr_loss = train(args, train_dataset, model, tokenizer)
            
            evaluate(args, model, tokenizer)
            save_model_dict(model, args.checkpoint_dir+ 'finetuned_model.pth')
            print('tmp/mrpc/LT/ is populated with finetune epoch checkpoints.')
            exit()
        else:
            finetuned_model_state_dict = torch.load(args.checkpoint_dir+ 'finetuned_model.pth')
        ...
        for p_step in range(pruning_steps):
            ...
            print('Pruned 10 percent of remaining weights.')
            zero = see_weight_rate(model)
            print('zero rate', zero)
            load_weight = torch.load(args.output_dir +'/weight' + str(rewind_epoch) + '.pt', map_location=args.device)
            model_dict = model.state_dict()
            model_dict.update(load_weight)
            model.load_state_dict(model_dict)
            print('Rewinding from epoch ' + str(rewind_epoch))

            global_step, tr_loss = train(args, train_dataset, model, tokenizer)
            logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
            save_model_dict(model, args.checkpoint_dir+ 'finetuned' + str(rewind_epoch) + '_' + str(p_step) + '.pth')
